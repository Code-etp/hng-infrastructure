...
# Generating keypair
resource "aws_key_pair" "hng_ssh-key" {
  key_name   = "hng_ssh-key"
  public_key = file("~/.ssh/hng_ssh-key.pub")
}
...
resource "aws_instance" "todo-app" {
  ...
  key_name        = aws_key_pair.hng_ssh-key.id
  tags = {
    Name = "todo-app"
  }
}
...
resource "aws_eip_association" "eip_assoc" {
  ...
  allocation_id = "eipalloc-03dd27d85cdc31385"  
}
...
resource "null_resource" "wait_for_instance" {
  ...
  provisioner "remote-exec" {
    connection {
      type        = "ssh"
      user        = "ubuntu"
      private_key = file("~/.ssh/hng_ssh-key")
      host        = "54.145.201.41"
      timeout     = "5m"
    }
...
resource "null_resource" "run_ansible" {
  depends_on = [
   ...
   provisioner "local-exec" {
    command = "ssh-keygen -R 54.145.201.41 && sleep 120 && ansible-playbook -i ansible-setup/inventory.yaml --extra-vars '@ansible-setup/terraform_vars.yml' ansible-setup/playbook.yaml"
  }
}